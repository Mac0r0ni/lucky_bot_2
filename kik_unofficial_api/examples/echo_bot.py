...
class EchoBot(KikClientCallback):
    ...
    def on_chat_message_received(self, chat_message: chatting.IncomingChatMessage):
        ...
        message_id = self.client.request_info_of_users(chat_message.from_jid)
        message_cache.append(message_id)
    ...
    def on_peer_info_received(self, response: PeersInfoResponse):
        ...
        if response.message_id in message_cache:
            print("Id Match")
        message_cache.remove(response.message_id)
        print(len(message_cache))
        for user in response.users:
            print(user.pic + "/orig.jpg")

    def on_xiphias_get_users_response(self, response: Union[UsersResponse, UsersByAliasResponse]):
        print("[+] Peer info: " + str(response.users[0].__dict__))

        print(response.message_id)
        print(x_message_cache[0])
